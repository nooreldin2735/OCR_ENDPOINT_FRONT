...
import { Search, Home, ChevronRight, Clock, LogOut, User as UserIcon, FileText } from "lucide-react";
...
export default function Navbar() {
    const { history, setIsHistoryOpen, isSidebarOpen, setIsSidebarOpen } = useOCRContext();
            ...
            {isSidebarOpen && (
                <>
                    ...
                    <motion.aside
                        ...
                        className="fixed top-0 left-0 h-full w-64 bg-accent border-r border-white/10 flex flex-col p-6 z-[60] text-white shadow-2xl"
                    >
                        {/* Brand Header */}
                        ...
                        <nav className="flex-1 flex flex-col gap-2">
                            <NavButton to="/" icon={<Home className="h-4 w-4" />} label="Home Dashboard" onClick={() => window.innerWidth < 1024 && setIsSidebarOpen(false)} />
                            <NavButton to="/ocr" icon={<Search className="h-4 w-4" />} label="OCR Intelligent Module" onClick={() => window.innerWidth < 1024 && setIsSidebarOpen(false)} />
...
function NavButton({ to, icon, label, onClick }: { to: string; icon: React.ReactNode; label: string; onClick?: () => void }) {
    return (
        <NavLink
            to={to}
            onClick={onClick}
